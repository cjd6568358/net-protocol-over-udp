...
const Uint16ArrayToUint8Array = (bufferViewer) => {
  data_8 = new Uint8Array(
    bufferViewer.buffer,
    bufferViewer.byteOffset,
    bufferViewer.byteLength
  );
};
...
const createHeaderPart = (randomID) => {
  ...
  return [
    ...
    Number(FLAG1),
    Number(FLAG2),
    ...strChunk(QDCOUNT, 8).map((item) => Number(`0b${item}`)),
    ...strChunk(ANCOUNT, 8).map((item) => Number(`0b${item}`)),
    ...strChunk(NSCOUNT, 8).map((item) => Number(`0b${item}`)),
    ...strChunk(ARCOUNT, 8).map((item) => Number(`0b${item}`)),
  ];
};
...
const createQuestionPart = (hostName) => {
  ...
  return Array.from([
    ...QNAME,
    ...strChunk(QTYPE, 8).map((item) => Number(`0b${item}`)),
    ...strChunk(QCLASS, 8).map((item) => Number(`0b${item}`)),
  ]);
};

const createReqBuffer = (hostName, randomID) => {
  return Uint8Array.from([
    ...createHeaderPart(randomID),
    ...createQuestionPart(hostName),
  ]);
};
...
const verifyResBuffer = (ID, reqLen, buffer) => {
  ...
  if (
    ID !=
    Number(
      `0x${Number(buffer[0]).toString(16)}${Number(buffer[1]).toString(16)}`
    )
  ) {
    return {
      code: 0,
  ...
  // FLAGS
  let FLAGS = `${Number(buffer[2]).toString(2)}${Number(buffer[3]).toString(
    2
  )}`;
  // 响应标志QR
  const QR = FLAGS[0];
